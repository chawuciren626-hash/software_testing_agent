...
#### 生成溯源：来源与可信度标记（`provenance.py` · 写在产物里）

用例文件常被单独发出去评审、贴进缺陷单、交给外包执行。**一旦它离开 `artifacts/`，
`run_meta.json` 里的生成方式就跟它脱节了** —— 读的人看到一张用例表，无从判断：

- 这是 LLM 写的，还是规则版模板套出来的？
- 如果是规则版，**是因为没配 key，还是 LLM 调用失败降级了**？
- 用了哪个模型？注入了哪些上下文（情景记忆 / 项目知识）？

**「没配 key 走规则版」和「LLM 超时降级成规则版」长得一模一样，但前者是预期行为，
后者意味着这次质量低于预期、值得查。** 所以标记**写在 `cases.md` 里**，跟着文件走：
...
可信度**不编百分比**：定性给档位，定量只引用实测的结构质量分。

| 模式 | 可信度 | 必须人工做什么 |
|---|---|---|
| 规则版 | 低 | 步骤/预期是模板占位，必须逐条细化 |
| LLM 增强 | 中 | 可能引入需求之外的假设，需核对业务口径 |
| LLM 多步自审 | 中高 | 同上；多一轮自评审，覆盖度实测更高 |
| 任一模式 + **降级** | 低（降级） | LLM 没干活，产物等同规则版，**要查原因** |

```bash
.venv/Scripts/python extensions/requirements_to_cases/provenance.py projects/mall-admin/artifacts/cases.md
```

降级发生时：CLI 打印告警、报告「生成模式」标红并写明原因、控制台生成后提示⚠️。
标记被渲染器吃掉时会从文本行**残缺回读**并自曝 `partial=True`，不假装完整。
