...
- **真实生效的 CI**：根目录 `.github/workflows/ci.yml`（GitHub **只读仓库根** —— 旧模板放在扩展目录里
  从未执行过）。自有单测为硬门禁（失败即红，无 `|| true`），基座遗留测试为非门禁（不拿第三方代码问题卡交付），
  项目级门禁在未配置被测环境时**显式警告并跳过，不静默判绿**；`extensions/reporting/gate_notify.py`
  把三道门禁汇总成**三态结论**（通过 / 未通过 / **未执行**）后发钉钉 + 163 邮件。
