...
**已停用的项目默认不计入**（目录下有 `.disabled` 标记，即控制台里"停用"过的项目），
与 `load_projects()` / 跨项目看板 / 控制台项目页保持一致。跳过的项目名会写进摘要，
**不静默隐藏**。需要一并统计时加 `--include-disabled`（控制台对应 `?include_disabled=1`）。

> 为什么必须一致：停用项目若还留着上次的失败产物，会持续把门禁拖红，
> 而项目列表里又看不到它（列表同样读了这个标记）→ 变成一个**无法解释的红色**，
> 最后的结果是团队学会忽略红色。

三个容易踩的坑，都是实际写 CI 时踩出来的：
...
3. **"未配置"和"未执行"是两码事。** 项目没声明 `web.yaml` 记「未配置」不参与判定（防无谓红）；
   声明了但没跑过记「未执行」算门禁未达成（防假绿）。混为一谈会同时产生假绿和假红。
