...
> - **§4.6 拆大文件：本轮不做**（见下）。
>
> **补充落地（2026-09-15，序 8 / §10 #2 效率口径的机制前提）**——注册表机制**扩展了两项声明**，
> 都是为让"未执行"在统计上可辨认：
>
> - **`Stage.enabled`（本次该不该跑）**：判否时**整段跳过**，`run` 根本不被调用。
>   `perf` / `web` / `agentic` 三个可选阶段的开关从"阶段体内进去先 `return`"**上移到注册表声明**。
>   这不是洁癖：写在体内的话，"没跑"与"跑得飞快"在耗时统计里**都是 ≈0 秒**，
>   报告上看起来像"这个能力很快"，实际是"这个能力根本没发生"。
> - **`run_stages(on_stage=...)` 改为"结束后钩子"**：签名 `(StageOutcome) -> None`，
>   在**每个阶段结束后**调用一次（成功 / 失败 / **跳过**都调）。
>   `StageOutcome.seconds is None` ⇔ 未执行；失败阶段**照样带实测耗时**
>   （抹掉最慢的失败路径只会让数字更好看）。
>   回调自身抛异常 → **记日志后忽略**：统计出问题绝不能改变业务结果，但也不静默吞掉。
> - **`on_stage` 只做"如实汇报"，不认识"效率"**：汇总、口径与"测不到什么"的声明
>   全在 `extensions/common/timing.py`（口径唯一）。
> - **等价性 oracle 未受污染**：新增的两项都是**可选声明**，`resolve_order` 结果不变 ——
>   `test_pipeline_registry.py` 里那份"冻结自序 6 HEAD 的顺序字面量"**没有被改动**。
...
2. ✅ **"效率提升"缺可达量化口径**【2026-09-15 已定口径（序 8）】
   - **原问题**：需求里"提升测试效率"没有任何测量方式，无法验收，也无法证明 S1 之后变好了。
     建议补一个定义（例如"从需求到可执行回归的耗时"或"人工编写用例条数下降比例"）。
   - **口径选定（用户拍板，2026-09-15）**：在三个候选里选 **A · 端到端耗时分解** ——
     即"一次 `run` 的**进程内墙钟耗时**，按阶段分解为明细 + 合计"。
     另两个候选（B 产出密度 / C 人工替代率）未采用：B 的数量不等于质量、
     需额外的质量归一化；C 的"人工基线"只能靠人填，系统内无从测量。
   - **唯一定义处**：`extensions/common/timing.py`（**零依赖**）。口径常量 `SPEC = "timing/1"`、
     `SCOPE = "需求 → 报告生成"`；`StageTime` / `from_outcomes` / `total_seconds` /
     `build` / `load` / `format_seconds` / `summary_line` / `render_lines`。
   - **三条纪律（沿用项目既有判据）**：① **未执行 ≠ 0 秒** —— 未开启的阶段记
     `seconds=None` 并落入 `skipped`，绝不记 `0.000`（同 CI 门禁三态的「未执行不是绿」）；
     ② **失败阶段照实计时**；③ **算不出就不猜** —— 一个阶段都没测到则合计为 `None`，
     不用 `0` 冒充。`load()` 对形状不符的载荷一律返回 `None`（**缺字段不算数**）。
   - **明确不承诺测量（写死在 `NOT_MEASURED`，逐条列出而非留白）**：
     **人力节省 / 人工替代率**（需外部工时基线，系统内无法测量）、
     **质量**（耗时短 ≠ 效率高；用例好坏由结构质量分另行评价）。
     载荷另带 `excluded`（口径范围内但本次不计）：`report` 阶段自身渲染耗时。
   - **落点**：`run_meta.json` 的 `timing` 字段（由 `_stage_report` **先定格口径、再渲染**——
     报告是持久化产物的渲染器，口径必须与其它卡片同源）+ 报告「效率口径」卡片
     （未执行阶段显示「未执行」而非 0）+ CLI 收尾一行摘要。
   - **守护**：`tests/test_timing.py`（**42 例，已进 CI 硬门禁白名单**）+ 
     `test_pipeline_registry.py` 新增 6 例（`enabled` 跳过 / 结束后钩子 / 失败照计时 / 回调异常不伤业务）。
     **变异验证 6 项全部红在预期粒度**：① 未执行记成 `0.0` → 2 例红；② 摘掉注册表的
     `enabled` 声明 → 2 例红（声明 + 行为）；③ 失败阶段不汇报耗时 → 1 例红；
     ④ 合计把 `None` 当 0 → 1 例红；⑤ `format_seconds(None)` 返回 `0ms` → 2 例红；
     ⑥ 报告"先渲染再定格口径" → 1 例红。
   - **门禁**：CI 硬门禁 **743 passed**（695 + 48）；基座作业 **837 passed / 5 skipped**。
   - **仍未知（不假装全覆盖）**：本口径只覆盖 **`run` 全流程**；`regression` / `rerun` /
     `explore` 等入口未接入（它们不写 `run_meta`，故无落点）。跨轮趋势化
     （把历次 `timing` 画成曲线）**未做** —— 但载荷已按固定形状落盘，接趋势时无需回到生成侧改代码。
