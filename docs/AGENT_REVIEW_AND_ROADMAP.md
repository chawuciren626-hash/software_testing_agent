...
- 验收：CI 跑通；门禁在"环境不可达"场景稳定判不绿。
  → **已验收（2026-09-14 核对）**：CI 于 run #18（`cf5e327`）首次全绿，四个作业均 success，
  此前 15 次全红（根因是两处跨平台缺陷，详见下方"CI 全红闭环"）；
  "环境不可达不判绿"有单测覆盖且 CI 已实际执行。
...
- [x] ~~S2 长期记忆~~ → 已落地（2026-09-11）：`extensions/memory/knowledge.py` —— 项目知识库
...
- 验收：无 key 时能力 == Phase0；有 key 时用例覆盖度/可执行性 L3 评分 ≥ 基线。
  → **半验收（2026-09-14 核对）**：前半（无 key == Phase0）成立，规则版降级链路有单测与 CI 覆盖；
  后半（有 key 时 L3 评分 ≥ 基线）**未做** —— 需先落地 S1 / "默认走 LLM"，
  再用 `tests/eval/llm_judge.py --repeat N` 跑基线与对比（单次方差大会得出相反结论，必须重复取均值）。

### Phase 2 — 自主进化（约 3 周）
- [x] **情景记忆回灌：失败根因 → 重点覆盖清单 → 用例自优化** → 已落地（2026-09-14）：
      `extensions/memory/focus.py`。`lessons.py` 原本只做到"聚类成重点清单并**注入**生成提示"，
      缺的是闭环后半段 —— **注入之后到底覆盖到没有，没人验证**，这等于把"提示词发出去了"当成
      "结果达成了"，是假绿的另一种形态。
      - 校验：生成完用例后，逐条比对重点清单（与 knowledge 同一套 2-gram 口径，阈值 0.34）。
      - 补齐：缺的补成**骨架用例**（标题带【回灌】+ 历史失败次数，步骤/预期注明需人工细化），
        **不编业务细节**。
      - 持久化：补齐行落到 `projects/<id>/focus_supplement.md`，每轮生成后自动并入 ——
        cases.md 每轮从需求重生成，不持久化就永远原地打转。
      - 三个数字**必须分开报**：`native`（生成即覆盖）/ `persisted`（沿用上轮回灌）/
        `backfilled`（本轮新增补齐）。合并成一个"覆盖率"，补齐就会冒充成模型学会了。
      - 不参与门禁判定（与 trend_diff / 质量分一致的克制），只做展示与趋势。
      ⚠️ 诚实边界：清单项高度相似时会一起被同一条用例判为已覆盖（实测 `错误密码A`/`错误密码B`
      被同一条含"密码"的用例同时判覆盖），所以覆盖率只能当**粗粒度信号**，
      已写进模块 docstring 并用测试固化（哪天换成语义匹配这条测试会变红提醒更新文档）。
      实测 zz-fail 连跑两轮：第 1 轮"3/6 生成即覆盖 + 3 条本轮新增补齐"，
      第 2 轮"3/6 生成即覆盖 + 3 条沿用上轮" —— 覆盖确实跨轮累积，且 native 不虚高。
      测试 20 项 + 两处变异验证（阈值降到 0、取消并回上轮补齐行）均已确认变红。
...
- 验收：同一项目重复 run，易错场景覆盖率单调上升。
  → **已验收（2026-09-14）**：zz-fail 连跑两轮，总覆盖由"3/6 + 本轮补齐 3"变为
  "3/6 + 沿用上轮 3"，**覆盖确实跨轮累积、不再回退**。
  注意区分两个指标：`native`（生成即覆盖）在规则版下**不会**自己涨（模板不会凭空写出业务场景），
  会涨的是**总覆盖率**（靠回灌补齐并持久化）。别拿 native 当验收口径 —— 那是模型能力的指标。
