...
- [ ] ~~S2 长期记忆~~ → 已落地（2026-09-11）：`extensions/memory/knowledge.py` —— 项目知识库
      ...
      每次生成用例前按**关键词重合度**挑出与本次需求相关的 Top-K 段注入（top_k=3，
      段落分数低于最高分 30% 即丢弃 —— **宁可少给，不能给错**，无关上下文会让 LLM 写歪）。
      ⚠️ 诚实边界：是**关键词/子串匹配（中文 2-gram）**，不是语义检索；
      认不出"登录凭证"和"用户名密码"是同一件事，已在 README 与控制台面板写明。
      接入：create 生成带注释模板、控制台「项目知识」页可直接编辑并**显示命中词**
      （写了却不知道有没有被用上，是最容易让人放弃维护它的情况）、
      run 时与 lessons 合并注入、run_meta 分别记 `lessons_injected`/`knowledge_injected`。
      踩坑：① 没有相对门槛时"只沾到一个共同词"的无关段会被凑满 top_k；
      ② 跑全流程前的落盘一开始漏了 knowledge，表现是"改了没效果"。
      测试 22 项 + 控制台 7 项 + 浏览器 1 项；变异（取消门槛）已确认会让测试变红。
...
- [x] **失败项新旧对比**：`extensions/reporting/trend_diff.py` —— 本次失败项 vs 上一次执行快照，
      分类 regressed / new / persistent / flaky / recovered / unknown，并记录**连续失败次数**。
      存在的意义不是再列一遍失败，而是回答**先看哪一个**（警报疲劳：长期同样的红会被习惯性忽略）。
      三条克制：① 上次全 SKIP（环境不可达）**不作基线**，否则全部失败都成了"新增"，那就是噪音；
      ② **不参与门禁判定**，只做排序提示；③ SKIP 不进胜负序列（会污染连续计数）。
      缺陷草稿会带上这些标签，并在**同级严重度内**按 回归 > 新增 > 持续失败 重排
      （严重程度仍是主排序，S1 不会被"刚回归的 S2"挤到后面）。
      ⚠️ 顺序要求：`_step_diff` 必须在写本次快照（`insert_snapshot`）**之前**调用，
      否则历史最新一条就是本次自己，自己跟自己比，结论永远是"没有新增失败"。
      接入面：run / regression 自动生成 + 报告卡片 + 控制台项目卡「新增失败」徽标与「新旧对比」页。
      测试 25 项 + 控制台 6 项 + 浏览器 1 项；三处变异（抖动误判回归、基线检查失效、
      伪对照自己）均已确认会让对应测试变红。
- [x] **L3 评测常态化**：`extensions/requirements_to_cases/case_quality.py` —— 每次 run 自动算
      **用例结构质量分**（需求覆盖/三类齐备/可执行性/具体性/去重，加权 0-100），
      ...
      三条诚实性约定：① **结构分 ≠ 质量判定**（只看形式完整性，语义质量仍靠 llm_judge 抽样）；
      ② **默认不做硬门禁**（结构分可注水刷高，卡 CI 等于鼓励刷分，正确用法是看趋势）；
      ③ **算不出就不猜**（维度记「未计分」+ 说明，总分按剩余维度重新归一化）。
      实测：mall-admin 规则版 95 分（具体性 47——模板只写"最小/最大/临界"这类占位描述），
      砍掉 2/3 用例后掉到 68（环比 ↓27）——趋势信号确实可用。
